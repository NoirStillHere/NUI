...
function NoirUI:CreateWindow(settings)
    ...
    -- WINDOW FUNCTIONS
    -- ============================================
    function windowRef:SetAccent(color)
        local newAccent = color or ACCENT
        if MainStroke then MainStroke.Color = newAccent end
        local glowStroke = Main and Main:FindFirstChild("GlowStroke")
        if glowStroke then glowStroke.Color = newAccent end
        for _, btn in pairs(allTabButtons) do
            local stroke = btn:FindFirstChildWhichIsA("UIStroke")
            if stroke and stroke.Name ~= "GlowStroke" then stroke.Color = newAccent end
            local glow = btn:FindFirstChild("GlowStroke")
            if glow then glow.Color = newAccent end
            local label = btn:FindFirstChild("TextLabel")
            if label and label.TextColor3 == ACCENT then label.TextColor3 = newAccent end
        end
        if LoadingBar then LoadingBar.BackgroundColor3 = newAccent end
        ACCENT = newAccent
        return true
    end
    
    function windowRef:SetMainBackground(color, transparency)
        if Main then
            if color then
                Main.BackgroundColor3 = color
                if useAutoContrast and Title then
                    Title.TextColor3 = GetContrastColor(color)
                end
                if useAutoContrast and Description then
                    Description.TextColor3 = GetContrastColor(color)
                end
            end
    ...
    function windowRef:SetBlur(amount)
        if Main then
            local blurWrapper = Main:FindFirstChild("BlurWrapper")
            if blurWrapper then
                local blur = blurWrapper:FindFirstChild("BlurOverlay")
                if blur then
                    blur.BackgroundTransparency = 1 - math.clamp(amount, 0, 1)
                end
            end
        end
        return true
    ...
    function windowRef:SetSidebarBackground(color, transparency)
        if Side then
            if color then Side.BackgroundColor3 = color end
            if transparency ~= nil then Side.BackgroundTransparency = transparency end
        end
        return true
    end
    
    function windowRef:SetTabBackground(color)
        for _, btn in pairs(allTabButtons) do
            if btn then
                btn.BackgroundColor3 = color or Color3.fromRGB(20, 20, 20)
                local label = btn:FindFirstChild("TextLabel")
                if label and useAutoContrast then
                    label.TextColor3 = GetContrastColor(btn.BackgroundColor3)
                end
            end
        end
        return true
    ...
    function windowRef:SetElementBackground(color)
        for _, tab in pairs(windowTabs) do
            if tab.Elements then
                for _, elem in pairs(tab.Elements) do
                    if elem:IsA("Frame") or elem:IsA("TextButton") then
                        if elem.BackgroundColor3 ~= Color3.fromRGB(0,0,0) then
                            elem.BackgroundColor3 = color or Color3.fromRGB(25, 25, 35)
                        end
                    end
                end
            end
    ...
    function windowRef:SetFloatIcon(iconName)
        if TBtn then
            local clipGroup = TBtn:FindFirstChild("ClipGroup")
            if clipGroup then
                local icon = clipGroup:FindFirstChild("ImageLabel") or clipGroup:FindFirstChild("TextLabel")
                if icon then
                    local resolved = ResolveIcon(iconName)
                    if resolved then
                        if icon:IsA("ImageLabel") then icon.Image = resolved
                        else icon.Text = iconName end
                    end
                end
            end
        end
    ...
    function windowRef:SetTitle(title)
        if Title then Title.Text = title or "NOIR HUB" end
        return true
    end
    
    function windowRef:SetDescription(desc)
        if Description then Description.Text = desc or "NoirUI V3" end
        return true
    end
    
    function windowRef:SetBackgroundImage(imageId, transparency)
        if Main then
            local bgImage = Main:FindFirstChild("_BackgroundImage")
            if bgImage then
                if imageId then bgImage.Image = imageId end
                if transparency ~= nil then bgImage.ImageTransparency = transparency end
            end
        end
        return true
    end
    
    function windowRef:SetFloatBackgroundImage(imageId, transparency)
        if TBtn then
            local clipGroup = TBtn:FindFirstChild("ClipGroup")
            if clipGroup then
                local bgImage = clipGroup:FindFirstChild("BackgroundImage")
                if bgImage then
                    if imageId then bgImage.Image = imageId end
                    if transparency ~= nil then bgImage.ImageTransparency = transparency end
                end
    ...
    function windowRef:ApplyTheme(themeName)
        local theme = NoirUI.Themes[themeName]
        if not theme then
            NoirUI:Notify("Theme", "Không tìm thấy theme: " .. tostring(themeName), "x", "Error")
            return false
        end
        
        if theme.accent then windowRef:SetAccent(theme.accent) end
        if theme.mainBg then windowRef:SetMainBackground(theme.mainBg, 0) end
        if theme.mainBgImage then windowRef:SetBackgroundImage(theme.mainBgImage, 0.5) end
        if theme.floatBgImage then windowRef:SetFloatBackgroundImage(theme.floatBgImage, 0.5) end
        if theme.blur ~= nil then windowRef:SetBlur(theme.blur) end
        if theme.elementBg then windowRef:SetElementBackground(theme.elementBg) end
        if theme.sidebarBg then windowRef:SetSidebarBackground(theme.sidebarBg, theme.sidebarTrans or 0.8) end
        if theme.tabBg then windowRef:SetTabBackground(theme.tabBg) end
        if theme.description then windowRef:SetDescription(theme.description) end
        if Title and theme.textColor then Title.TextColor3 = theme.textColor end
        ...
        if theme.vibeSound then
            ...
            sound.SoundId = theme.vibeSound
            sound.Volume = 0.3
            sound.Looped = true
            sound.Parent = game:GetService("CoreGui")
            sound:Play()
            vibeSound = sound
            windowRef._vibeSound = sound
        end
        
        currentTheme = themeName
        NoirUI:Notify("🎨 Theme", "Đã áp dụng: " .. theme.name, "palette", "Success")
        return true
    end
    
    function windowRef:StopVibeSound()
        if vibeSound then
            vibeSound:Stop()
            vibeSound:Destroy()
            vibeSound = nil
            windowRef._vibeSound = nil
        end
    end
    
    function windowRef:GetCurrentTheme()
        return currentTheme
    end
    
    function windowRef:GetAccent()
        return ACCENT
    end
    
    function windowRef:PushHistory(state)
        NoirUI:PushHistory(state)
    end
    
    function windowRef:Undo(callback)
        NoirUI:Undo(callback)
    end
    
    function windowRef:Redo(callback)
        NoirUI:Redo(callback)
    end
    
    function windowRef:Notify(title, message, iconName, soundType)
        NoirUI:Notify(title, message, iconName, soundType)
    end
    
    function windowRef:Destroy()
        if vibeSound then
            vibeSound:Stop()
            vibeSound:Destroy()
            vibeSound = nil
        ...
        table.remove(NoirUI._windows, table.find(NoirUI._windows, windowRef))
    end
    
    -- ============================================
    -- SOUND FUNCTIONS
    -- ============================================
    function windowRef:SetCustomSound(soundType, soundId)
    ...
    function windowRef:ToggleSound(enabled)
        SoundSettings.Enabled = enabled
        windowRef:Notify("🔊 Sound", "Đã " .. (enabled and "bật" or "tắt") .. " âm thanh", enabled and "volume-2" or "volume-x")
    end
    
    function windowRef:SetVolume(volume)
        SoundSettings.Volume = math.clamp(volume, 0, 1)
        windowRef:Notify("🔊 Volume", "Âm lượng: " .. math.floor(volume * 100) .. "%", "volume-2")
    end
    ...
    local function PlaySound(soundType)
        ...
        NoirUI.Sound:Play(soundId, SoundSettings.Volume)
    end
    ...
    settings.UseRipple = settings.UseRipple or false
    settings.UseParticles = settings.UseParticles or false
    settings.UseNeon = settings.UseNeon or false
    settings.UseGlitch = settings.UseGlitch or false
    settings.UsePop = settings.UsePop or false
    settings.UseBounce = settings.UseBounce or false
    settings.UseSlide = settings.UseSlide or false
    settings.UseFloating = settings.UseFloating or false
    settings.UseConfetti = settings.UseConfetti or false
    settings.UseTyping = settings.UseTyping or false
    settings.UseGlow = settings.UseGlow or false
    settings.ParticleCount = settings.ParticleCount or 30
    
    settings.DefaultTheme = settings.DefaultTheme or "default"
    ...
    if settings.BackgroundMusic then
        NoirUI.Sound:InitBackgroundMusic(settings.BackgroundMusic)
    end
    ...
    NoirUI.Effects.AddGlowStroke(Main, ACCENT, 2, 6, 0.7)
    ...
    if settings.UseGlow then
        local glow, conn = NoirUI.Effects.CreateUIGlow(Main, ACCENT, 0.3, ScreenGui)
        table.insert(NoirUI.EffectConnections, conn)
    end
    if settings.UseParticles then
        local conn = NoirUI.Effects.CreateParticles(Main, ACCENT, settings.ParticleCount)
        table.insert(NoirUI.EffectConnections, conn)
    end
    if settings.UseNeon then
        local conn = NoirUI.Effects.CreateNeonPulse(Main, ACCENT, 0.3)
        table.insert(NoirUI.EffectConnections, conn)
    end
    ...
    if settings.UseGlow then
        local glow, conn = NoirUI.Effects.CreateUIGlow(TBtn, ACCENT, 0.4, ScreenGui)
        table.insert(NoirUI.EffectConnections, conn)
    end
    if settings.UseParticles then
        local conn = NoirUI.Effects.CreateParticles(TBtn, ACCENT, 10)
        table.insert(NoirUI.EffectConnections, conn)
    end
    if settings.UseNeon then
        local conn = NoirUI.Effects.CreateNeonPulse(TBtn, ACCENT, 0.3)
        table.insert(NoirUI.EffectConnections, conn)
    end
    ...
    TS.Color = ACCENT
    TS.Thickness = 2
    NoirUI.Effects.AddGlowStroke(TBtn, ACCENT, 2, 5, 0.75)
    ...
    NoirUI.Effects.AddGlowStroke(LoadingFrame, ACCENT, 2, 5, 0.75)
    AddBlurOverlay(LoadingFrame, settings.LoadingBlur)
    
    if settings.UseGlow then
        local glow, conn = NoirUI.Effects.CreateUIGlow(LoadingFrame, ACCENT, 0.3, ScreenGui)
        table.insert(NoirUI.EffectConnections, conn)
    end
    if settings.UseNeon then
        local conn = NoirUI.Effects.CreateNeonPulse(LoadingFrame, ACCENT, 0.3)
        table.insert(NoirUI.EffectConnections, conn)
    end
    ...
    if settings.UseGlow then
        NoirUI.Effects.CreateGlowText(LoadingTitle, {
            Color3.fromRGB(200, 200, 200),
            Color3.fromRGB(255, 255, 255),
            Color3.fromRGB(200, 200, 200)
        }, 1.5)
    end
    if settings.UseGlitch then
        local conn = NoirUI.Effects.CreateGlitch(LoadingTitle, 3)
        table.insert(NoirUI.EffectConnections, conn)
    end
    if settings.UseTyping then
        NoirUI.Effects.CreateTyping(LoadingTitle, LoadingTitle.Text, 0.05)
    end
    ...
    if settings.UseNeon then
        NoirUI.Effects.CreateNeonPulse(LoadingBar, ACCENT, 0.3)
    end
        ...
        loadingConnection = RunService.RenderStepped:Connect(function()
            ...
            if percent >= 1 then
                ...
                if settings.UseBounce then
                    NoirUI.Effects.Bounce(Main, 20, 0.5)
                end
                if settings.UseConfetti then
                    NoirUI.Effects.CreateConfetti(Main, 20)
                end
                PlaySound("Open")
                if settings.BackgroundMusic and settings.BackgroundMusic.AutoPlay then
                    NoirUI.Sound:StartMusic()
                end
                if settings.DefaultTheme then
                    task.wait(0.2)
                    windowRef:ApplyTheme(settings.DefaultTheme)
                end
            end
        end)
    ...
    if settings.KeySystem then
        ...
        if not KeySolved then
            ...
            kstr.Thickness = 2
            kstr.Color = ACCENT
            NoirUI.Effects.AddGlowStroke(KUI, ACCENT, 2, 8, 0.6)
            AddBlurOverlay(KUI, settings.KeyBlur)
            MakeDraggable(KUI)
            if settings.UseGlow then
                local glow, conn = NoirUI.Effects.CreateUIGlow(KUI, ACCENT, 0.3, ScreenGui)
                table.insert(NoirUI.EffectConnections, conn)
            end
            if settings.UseNeon then
                local conn = NoirUI.Effects.CreateNeonPulse(KUI, ACCENT, 0.3)
                table.insert(NoirUI.EffectConnections, conn)
            end
            ...
            if settings.UseGlow then
                NoirUI.Effects.CreateGlowText(KT, {
                    Color3.fromRGB(200, 200, 200),
                    Color3.fromRGB(255, 255, 255),
                    Color3.fromRGB(200, 200, 200)
                }, 1.5)
            ...
            if settings.UseGlitch then
                local conn = NoirUI.Effects.CreateGlitch(KT, 3)
                table.insert(NoirUI.EffectConnections, conn)
            end
            ...
            if settings.UseRipple then
                NoirUI.Effects.CreateRipple(KB, ACCENT, 0.3)
            end
            
            KB.MouseButton1Click:Connect(function()
                CreateClickScaleEffect(KB)
                if settings.UsePop then
                    NoirUI.Effects.Pop(KB, 1.1)
                end
    ...
    if settings.UseGlow then
        NoirUI.Effects.CreateGlowText(Title, {
            Color3.fromRGB(200, 200, 200),
            Color3.fromRGB(255, 255, 255),
            Color3.fromRGB(200, 200, 200)
        }, 1.5)
    ...
    if settings.UseGlitch then
        local conn = NoirUI.Effects.CreateGlitch(Title, 3)
        table.insert(NoirUI.EffectConnections, conn)
    end
    if settings.UseNeon then
        local conn = NoirUI.Effects.CreateNeonPulse(Title, ACCENT, 0.3)
        table.insert(NoirUI.EffectConnections, conn)
    end
    ...
    if settings.UseTyping then
        NoirUI.Effects.CreateTyping(Description, Description.Text, 0.03)
    end
    ...
    local function TopB(txt, col, cb)
        ...
        CreateHoverEffect(b)
        if settings.UseRipple then
            NoirUI.Effects.CreateRipple(b, ACCENT, 0.3)
        end
        if settings.UseNeon then
            NoirUI.Effects.CreateNeonPulse(b, ACCENT, 0.3)
        end
        b.MouseButton1Click:Connect(function()
            CreateClickScaleEffect(b)
            if settings.UsePop then
                NoirUI.Effects.Pop(b, 1.1)
            end
            cb()
        end)
        return b
    ...
    TopB("X", Color3.fromRGB(255, 100, 100), function()
        ...
        s.Color = ACCENT
        s.Thickness = 2
        NoirUI.Effects.AddGlowStroke(Conf, ACCENT, 2, 4, 0.85)
        AddBlurOverlay(Conf, settings.ConfirmBlur)
        if settings.UseGlow then
            local glow, conn = NoirUI.Effects.CreateUIGlow(Conf, ACCENT, 0.3, ScreenGui)
            table.insert(NoirUI.EffectConnections, conn)
        end
        if settings.UseNeon then
            local conn = NoirUI.Effects.CreateNeonPulse(Conf, ACCENT, 0.3)
            table.insert(NoirUI.EffectConnections, conn)
        end
        CreateSpringEffect(Conf)
        ...
        if settings.UseRipple then
            NoirUI.Effects.CreateRipple(cbtn, ACCENT, 0.3)
        end
        ...
        if settings.UseRipple then
            NoirUI.Effects.CreateRipple(fbtn, ACCENT, 0.3)
        end
    ...
    NoirUI.Effects.AddGlowStroke(Side, ACCENT, 1, 1, 0.85)
    ...
    if settings.UseNeon then
        NoirUI.Effects.CreateNeonPulse(AI, ACCENT, 0.5)
    end
    ...
    NoirUI.Effects.AddGlowStroke(Cont, ACCENT, 1, 1, 0.7)
    ...
    TBtn.MouseButton1Click:Connect(function()
        ...
        if settings.UsePop then
            NoirUI.Effects.Pop(TBtn, 1.2)
        end
        if settings.UseRipple then
            NoirUI.Effects.CreateRipple(TBtn, ACCENT, 0.3)
        end
        ...
        if not KeySolved and KUI and KUI.Parent then
            ...
            if Main.Visible then
                PlaySound("Close")
                if settings.UseSlide then
                    local vp = Workspace.CurrentCamera.ViewportSize
                    TweenService:Create(Main, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                        Position = UDim2.new(0, -Main.AbsoluteSize.X - 50, 0, Main.Position.Y.Offset)
                    }):Play()
                    task.wait(0.3)
                    Main.Visible = false
                    Main.Position = mainDefaultPos
    ...
    function NoirUI:Notify(title, message, iconName, soundType)
        ...
        ns.Color = ACCENT
        NoirUI.Effects.AddGlowStroke(n, ACCENT, 2, 5, 0.75)
        AddBlurOverlay(n, settings.NotificationBlur)
        if settings.UseGlow then
            local glow, conn = NoirUI.Effects.CreateUIGlow(n, ACCENT, 0.2, ScreenGui)
            table.insert(NoirUI.EffectConnections, conn)
        end
        if settings.UseNeon then
            local conn = NoirUI.Effects.CreateNeonPulse(n, ACCENT, 0.3)
            table.insert(NoirUI.EffectConnections, conn)
        end
        SetupBackground(n, settings.NotificationBackground, settings.NotificationBackgroundColor, 0.25)
        CreateSpringEffect(n)
        if settings.UseSlide then
            NoirUI.Effects.SlideIn(n, "right", 0.4)
        end
        if settings.UseFloating then
            local conn = NoirUI.Effects.CreateFloatingAnimation(n, 3, 0.5)
            table.insert(NoirUI.EffectConnections, conn)
        end
        ...
        if settings.UseGlow then
            NoirUI.Effects.CreateGlowText(tl, {
                Color3.fromRGB(200, 200, 200),
                Color3.fromRGB(255, 255, 255),
                Color3.fromRGB(200, 200, 200)
            }, 1.5)
        ...
        if settings.UseTyping then
            NoirUI.Effects.CreateTyping(tl, title, 0.03)
        end
        if settings.UseGlitch then
            local conn = NoirUI.Effects.CreateGlitch(tl, 4)
            table.insert(NoirUI.EffectConnections, conn)
        end
        ...
        if settings.UseConfetti then
            NoirUI.Effects.CreateConfetti(n, 10)
        end
    ...
    function windowRef:CreateTabGroup(title, defaultOpen)
        ...
        if settings.UseRipple then
            NoirUI.Effects.CreateRipple(header, ACCENT, 0.3)
        end
        if settings.UseNeon then
            NoirUI.Effects.CreateNeonPulse(header, ACCENT, 0.3)
        end
        ...
        if settings.UseGlow then
            NoirUI.Effects.CreateGlowText(titleLabel, {
                Color3.fromRGB(200, 200, 200),
                Color3.fromRGB(255, 255, 255),
                Color3.fromRGB(200, 200, 200)
            }, 1.2)
        ...
        if settings.UseGlitch then
            local conn = NoirUI.Effects.CreateGlitch(titleLabel, 5)
            table.insert(NoirUI.EffectConnections, conn)
        end
    ...
    function windowRef:CreateTab(name, icon)
        ...
        tabStroke.ZIndex = 2
        local tabGlowStroke = NoirUI.Effects.AddGlowStroke(B, ACCENT, 2, 1, 0.9)
        ...
        if settings.UseGlow then
            NoirUI.Effects.CreateGlowText(BT, {
                Color3.fromRGB(200, 200, 200),
                Color3.fromRGB(255, 255, 255),
                Color3.fromRGB(200, 200, 200)
            }, 1)
        ...
        if settings.UseGlitch then
            local conn = NoirUI.Effects.CreateGlitch(BT, 4)
            table.insert(NoirUI.EffectConnections, conn)
        end
        if settings.UseTyping then
            NoirUI.Effects.CreateTyping(BT, name, 0.04)
        end
        ...
        NoirUI.Effects.AddGlowStroke(SearchFrame, ACCENT, 1, 1, 0.85)
        if settings.UseNeon then
            NoirUI.Effects.CreateNeonPulse(SearchFrame, ACCENT, 0.3)
        end
        ...
        local function selectThisTab()
            ...
            if settings.UseSlide then
                NoirUI.Effects.SlideIn(TabContainer, "left", 0.3)
            end
            if settings.UseConfetti then
                NoirUI.Effects.CreateConfetti(TabContainer, 10)
            end
            updateCanvas()
        end
        ...
        function Tab:CreateLabel(text, updateFunction)
            ...
            if settings.UseTyping and type(text) == "string" then
                NoirUI.Effects.CreateTyping(l, text, 0.03)
            end
            if settings.UseGlitch then
                local conn = NoirUI.Effects.CreateGlitch(l, 5)
                table.insert(NoirUI.EffectConnections, conn)
            end
        ...
        function Tab:CreateSection(title, noLine)
            ...
            if settings.UseRipple then
                NoirUI.Effects.CreateRipple(headerBtn, ACCENT, 0.3)
            end
            ...
            if settings.UseGlow then
                NoirUI.Effects.CreateGlowText(lbl, {
                    Color3.fromRGB(200, 200, 200),
                    Color3.fromRGB(255, 255, 255),
                    Color3.fromRGB(200, 200, 200)
                }, 1.2)
            ...
            if settings.UseGlitch then
                local conn = NoirUI.Effects.CreateGlitch(lbl, 5)
                table.insert(NoirUI.EffectConnections, conn)
            end
            ...
            headerBtn.MouseButton1Click:Connect(function()
                ...
                if settings.UsePop then
                    NoirUI.Effects.Pop(headerBtn, 1.1)
                end
            ...
            if not noLine then
                ...
                if settings.UseNeon then
                    NoirUI.Effects.CreateNeonPulse(line, ACCENT, 0.3)
                end
            end
        ...
        function Tab:CreateParagraph(opt)
            ...
            title.TextXAlignment = "Left"
            if settings.UseTyping then
                NoirUI.Effects.CreateTyping(title, opt.Title or "", 0.03)
            end
        ...
        function Tab:CreateButton(opt)
            ...
            b.MouseButton1Click:Connect(function()
                ...
                if settings.UseConfetti then
                    NoirUI.Effects.CreateConfetti(b, 15)
                end
                if settings.UseBounce then
                    NoirUI.Effects.Bounce(b, 5, 0.3)
                end
        ...
        function Tab:CreateToggle(opt)
            return NoirUI.Components.CreateToggle(Tab._currentSectionContent or ContentFrame, opt, {
                Accent = ACCENT,
                UseNeon = settings.UseNeon,
                UsePop = settings.UsePop,
                UseRipple = settings.UseRipple,
                AutoContrast = useAutoContrast,
                ElementBg = settings.ElementBackgroundColor
            })
        end
        
        function Tab:CreateSlider(opt)
            return NoirUI.Components.CreateSlider(Tab._currentSectionContent or ContentFrame, opt, {
                Accent = ACCENT,
                UseNeon = settings.UseNeon,
                AutoContrast = useAutoContrast
            })
        ...
        function Tab:CreateDropdown(opt)
            return NoirUI.Components.CreateDropdown(Tab._currentSectionContent or ContentFrame, opt, {
                Accent = ACCENT,
                AutoContrast = useAutoContrast,
                UseRipple = settings.UseRipple,
                UsePop = settings.UsePop,
                UseGlow = settings.UseGlow,
                ElementBg = settings.ElementBackgroundColor
            })
        end
        ...
        function Tab:CreateRunBox(opt)
            ...
            i.Position = UDim2.new(0, 10, 0, 0)
            i.BackgroundTransparency = 1
            i.PlaceholderText = opt.Placeholder or "Nhập: lệnh, loadstring('url'), or lua code"
            i.PlaceholderColor3 = Color3.fromRGB(150, 150, 150)
            i.Text = ""
            if useAutoContrast then
                i.TextColor3 = GetContrastColor(f.BackgroundColor3)
            ...
            if settings.UseRipple then
                NoirUI.Effects.CreateRipple(r, ACCENT, 0.3)
            end
            if settings.UseNeon then
                NoirUI.Effects.CreateNeonPulse(r, ACCENT, 0.3)
            end
            ...
            r.MouseButton1Click:Connect(function()
                ...
                if input:lower():match("loadstring") then
                    NoirUI:Notify("🚫 Không hỗ trợ", "loadstring đã bị vô hiệu hóa!", "x", "Error")
                    if opt.ClearOnExecute then
                        i.Text = ""
                    end
                    return
                ...
                if input:lower():match("require") then
                    NoirUI:Notify("🚫 Không hỗ trợ", "require đã bị vô hiệu hóa!", "x", "Error")
                    if opt.ClearOnExecute then
                        i.Text = ""
                    end
                    return
                ...
                if success then
                    ...
                    if execSuccess then
                        NoirUI:Notify("✅ Execute", "Code đã chạy thành công!", "check", "Success")
                    else
                        NoirUI:Notify("❌ Error", execErr or "Lỗi khi thực thi", nil, "Error")
                    end
                else
        ...
        -- ============================================
        -- NEW COMPONENTS (Icon Button, Grid, Badge, etc.)
        -- ============================================
        function Tab:CreateIconButton(iconName, pos, size, color, iconSize, iconColor, callback)
            return NoirUI.Components.CreateIconButton(
                Tab._currentSectionContent or ContentFrame,
                iconName, pos, size, color, iconSize, iconColor, callback,
                {Accent = ACCENT, UseNeon = settings.UseNeon, UsePop = settings.UsePop, UseConfetti = settings.UseConfetti}
            )
        end
        
        function Tab:CreateGrid(columns, cellSize, cellPadding, position, size)
            return NoirUI.Components.CreateGrid(
                Tab._currentSectionContent or ContentFrame,
                columns, cellSize, cellPadding, position, size
            )
        end
        
        function Tab:CreateBadge(text, color)
            return NoirUI.Components.CreateBadge(
                Tab._currentSectionContent or ContentFrame,
                text, color or ACCENT,
                {UseNeon = settings.UseNeon}
            )
        end
        
        function Tab:CreateSegmentedControl(options, default, callback)
            return NoirUI.Components.CreateSegmentedControl(
                Tab._currentSectionContent or ContentFrame,
                options, default, callback,
                {Accent = ACCENT, UseRipple = settings.UseRipple, UsePop = settings.UsePop}
            )
        end
        
        function Tab:CreateProgressBar(value, max, color)
            return NoirUI.Components.CreateProgressBar(
                Tab._currentSectionContent or ContentFrame,
                value, max, color or ACCENT,
                {UseNeon = settings.UseNeon}
            )
        end
        
        function Tab:CreateRadioGroup(options, default, callback)
            return NoirUI.Components.CreateRadioGroup(
                Tab._currentSectionContent or ContentFrame,
                options, default, callback,
                {Accent = ACCENT, UseRipple = settings.UseRipple, UsePop = settings.UsePop}
            )
        ...
        function Tab:CreateCard(opt)
            return NoirUI.Components.CreateCard(
                Tab._currentSectionContent or ContentFrame,
                opt,
                {Accent = ACCENT, UseGlow = settings.UseGlow, UseNeon = settings.UseNeon}
            )
        end
        
        function Tab:CreateLoadingSpinner(size)
            return NoirUI.Components.CreateLoadingSpinner(
                Tab._currentSectionContent or ContentFrame,
                size,
                {Accent = ACCENT, UseNeon = settings.UseNeon}
            )
        end
    ...
    windowRef._vibeSound = vibeSound
    windowRef._screen = ScreenGui
...
-- HISTORY SYSTEM
-- ============================================
function NoirUI:PushHistory(state)
    if #NoirUI.History.undo >= NoirUI.History.max then
        table.remove(NoirUI.History.undo, 1)
    end
    table.insert(NoirUI.History.undo, state)
    NoirUI.History.redo = {}
end

function NoirUI:Undo(callback)
    if #NoirUI.History.undo == 0 then 
        NoirUI:Notify("⚠️ Nothing to undo", "Không có gì để quay lại", "warning", "Error")
        return 
    end
    local state = table.remove(NoirUI.History.undo)
    table.insert(NoirUI.History.redo, state)
    if callback then callback(state) end
end

function NoirUI:Redo(callback)
    if #NoirUI.History.redo == 0 then 
        NoirUI:Notify("⚠️ Nothing to redo", "Không có gì để làm lại", "warning", "Error")
        return 
    end
    local state = table.remove(NoirUI.History.redo)
    table.insert(NoirUI.History.undo, state)
    if callback then callback(state) end
end
...
function NoirUI:Destroy()
    ...
    if NoirUI._windows then
        for _, win in pairs(NoirUI._windows) do
            if win and win._vibeSound then
                win._vibeSound:Stop()
                win._vibeSound:Destroy()
            end
            if win and win._screen then
                win._screen:Destroy()
            end
        end
        NoirUI._windows = {}
    ...
    for _, connection in pairs(NoirUI.EffectConnections) do
        pcall(function() connection:Disconnect() end)
    end
    ...
    NoirUI.ActiveConfirmFrame = nil
    NoirUI.Connections = {}
    NoirUI.EffectConnections = {}
    NoirUI.Glows = {}
    NoirUI.TabGroups = {}
    NoirUI.History = { max = 20, undo = {}, redo = {} }
end
